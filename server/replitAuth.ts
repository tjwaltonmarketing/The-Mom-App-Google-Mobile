// Replit Auth integration - supports Google, Apple, and Email/Password login
...
async function upsertReplitUser(claims: any) {
  ...
  if (existingUser) {
    ...
    return await storage.updateUserWithReplitAuth(existingUser.id, {
    ...
    // Create new user for Replit Auth
    return await storage.createReplitUser({
      email: claims["email"],
      replitUserId: claims["sub"],
