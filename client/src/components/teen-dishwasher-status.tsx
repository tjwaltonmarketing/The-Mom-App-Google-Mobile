...
export function TeenDishwasherStatus() {
  ...
  const updateDishwasherMutation = useMutation({
    mutationFn: async (isClean: boolean) => {
      return await apiRequest("PUT", "/api/teen/household-settings/dishwasher", { isClean });
    },
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ["/api/teen/household-settings"] });
      toast({
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <Label htmlFor="teen-dishwasher-status" className="text-base font-medium">
              Dirty / Clean
            </Label>
