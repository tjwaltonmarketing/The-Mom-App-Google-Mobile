...
export function MobileNav() {
  ...
  return (
    <nav className="lg:hidden bg-white dark:bg-card blue-light-filter:bg-card border-t border-gray-200 dark:border-border blue-light-filter:border-border fixed bottom-0 left-0 right-0 z-50 m-0 p-0 ios-safe-bottom">
      <div className="flex justify-around py-2 px-2">
        <Link href="/" className={`flex flex-col items-center py-2 px-2 flex-1 ${location === "/" ? "text-primary" : "text-gray-600 dark:text-gray-300 blue-light-filter:text-gray-600"}`}>
          <Home className="text-lg" />
          <span className="text-xs mt-1">Home</span>
        </Link>
        <Link href="/calendar" className={`flex flex-col items-center py-2 px-2 flex-1 ${location === "/calendar" ? "text-primary" : "text-gray-600 dark:text-gray-300 blue-light-filter:text-gray-600"}`}>
