...
## Changelog
- July 13, 2025. Teen mobile interface optimized: Hidden dark mode and blue light filter icons on mobile for cleaner interface, ensured "THE MOM APP" branding appears next to logo on all screen sizes. Teen notification system fully operational with SMS via Twilio. Email system architecture complete but blocked by Replit environment variable not updating with new SendGrid API key. SMS notifications working perfectly with timing: immediate assignment → 2hr before due → 15min past due → every 4hrs until completed. System includes quiet hours (10 PM - 8 AM), progressive messaging, and auto-stop when tasks completed.
